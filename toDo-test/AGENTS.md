...
- Ensure project initialization also enforces the `${MGMT_DIR}\toDo\` / `${MGMT_DIR}\errFix\` ordinal blank-file rules declared above, including the `${MGMT_DIR}\toDo\done\` and `${MGMT_DIR}\errFix\fixed\` subdir guidance (these rules must be present in project-local `AGENTS.md`).
